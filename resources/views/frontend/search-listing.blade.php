            ...
            <div class="my-md-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-3 flex-nowrap flex-xl-wrap overflow-auto overflow-xl-visble">
                        ...
                        <li class="breadcrumb-item flex-shrink-0 flex-xl-shrink-1" aria-current="page"><a href="#">{{ucwords($search)}}</a></li>
                    </ol>
                </nav>
            </div>
            <!-- End breadcrumb -->
        ...
        <div class="row mb-8">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                ...
                <div class="d-block d-md-flex flex-center-between mb-3">
                    <h3 class="font-size-25 mb-2 mb-md-0">{{ucwords($search)}}</h3>
                ...
                <div class="product-listing col-12">
                    ...
                    <div class="row">
                        @if(isset($request->sort))
                            @if($request->sort == 1)
                                @php $products = $results->orderBy('price'); @endphp <!-- Sort ascending -->
                            @elseif($request->sort == 2)
                                @php $products = $results->orderBy('price', 'DESC'); @endphp <!-- Sort descending -->
                            @endif
                        @endif

                        @if(isset($request->products))
                            @if($request->products == "all")
                                @php $products = $results->get(); @endphp
                            @else
                                @php $products = $results->paginate($request->products); @endphp
                            @endif
                        @else
                            @php $products = $results->paginate(24); @endphp
                        @endif
                        ...
                        @if(isset($request->sort))
                            @if($request->sort == 1)
                                @php $sorting = $products->sortBy('price'); @endphp <!-- Sort ascending -->
                            @elseif($request->sort == 2)
                                @php $sorting = $products->sortByDesc('price'); @endphp <!-- Sort descending -->
                            @endif
                        @endif
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card border-0 mb-4 mt-4">
                                <div class="alert alert-primary text-white mb-0" role="alert">
                                    No products found with this keyword. Here is a list of other products you might want to have a look.
                                </div>
                            </div>
                        ...
                        @if($request->products != 'all')
                            ...
                            <nav class="d-md-flex justify-content-between align-items-center border-top pt-3" aria-label="Page navigation example">
                                <div class="text-center text-md-left mb-3 mb-md-0">Showing {{ $sorting->count() }} results out of {{ $results->count() }}</div>
                                <ul class="pagination mb-0 pagination-shop justify-content-center justify-content-md-start">
                                    @php
                                    $startPage = max(1, $sorting->currentPage() - 3);
                                    $endPage = min($sorting->lastPage(), $sorting->currentPage() + 3);
                                    @endphp
                                    @for ($page = $startPage; $page <= $endPage; $page++) @if ($page==$sorting->currentPage())
                                        <li class="page-item"><a class="page-link current" href="javascript:void(0)">{{ $page }}</a></li>
                                        @else
                                        <li class="page-item"><a class="page-link" href="{{ $sorting->appends(request()->query())->url($page) }}">{{ $page }}</a></li>
                                        @endif
                                        @endfor
                                        @if ($sorting->hasMorePages())
                                        <li class="page-item"><a class="page-link" href="{{ $sorting->appends(request()->query())->nextPageUrl() }}">
